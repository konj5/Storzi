...
def init(jelka: Jelka):
    ...
    for i in range(št_snežink_x):
        for j in range(št_snežink_y):
            snežinke.append(
                Snežinka(
                    -math.pi + 2 * math.pi * ((i + random.uniform(-0.5, 0.5)) / št_snežink_x),
                    -radius + (1 + 2 * radius) * ((j + 1 + random.uniform(-0.5, 0.5)) / št_snežink_y),
                    horizontalna_hitrost_na_začetku,
                    max_vertikalna_hitrost,
                    radius,
