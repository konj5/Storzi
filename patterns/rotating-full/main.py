...
def callback(jelka: Jelka):
    plane = Plane(
        (0.5, 0.5, 0.5), (math.sin(jelka.frame / 40), 0, math.cos(jelka.frame / 40))
    )
    threshold = 10
    global c1, c2
    ...
    if jelka.frame % 150 == 0:
        ...
        c2hsv = (
            (c1hsv[0] * 360 + random.randint(80, 280) % 360) / 360.0,
            c1hsv[1],
            c1hsv[2],
        )
    ...
    for i in range(jelka.n):
        ...
        dcrtica = (
            pos[0] * plane.normal[0]
            + pos[1] * plane.normal[1]
            + pos[2] * plane.normal[2]
        )
